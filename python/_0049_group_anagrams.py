...
class TestGroupAnagrams(unittest.TestCase):
    ...
    def test_all_same_string(self):
        self.assertGroupsEqual(self.sol.groupAnagrams(["abc", "abc", "abc"]), [["abc", "abc", "abc"]])

    def test_no_anagrams(self):
        self.assertGroupsEqual(self.sol.groupAnagrams(["ab", "cd", "ef"]), [["ab"], ["cd"], ["ef"]])

    def test_single_chars_no_anagram(self):
        self.assertGroupsEqual(self.sol.groupAnagrams(["a", "b", "c"]), [["a"], ["b"], ["c"]])
    ...
    def test_palindromes_and_anagrams(self):
        self.assertGroupsEqual(self.sol.groupAnagrams(["aba", "baa", "aab"]), [["aba", "baa", "aab"]])

    def test_duplicates_in_groups_preserved(self):
        self.assertEqual(
            sorted([len(g) for g in self.sol.groupAnagrams(["b", "a", "a", "b", "c"])]),
            [1, 2, 2],
        )
